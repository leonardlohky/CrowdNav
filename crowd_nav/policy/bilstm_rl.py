...
class ValueNetwork1(nn.Module):
    def __init__(self, input_dim, self_state_dim, mlp_dims, lstm_hidden_dim):
        ...
        self.bilstm = nn.LSTM(input_dim, lstm_hidden_dim, batch_first=True, bidirectional=True)
...
class ValueNetwork2(nn.Module):
    def __init__(self, input_dim, self_state_dim, mlp1_dims, mlp_dims, lstm_hidden_dim):
        ...
        self.bilstm = nn.LSTM(mlp1_dims[-1], lstm_hidden_dim, batch_first=True, bidirectional=True)
        self.mlp = mlp(self_state_dim + lstm_hidden_dim, mlp_dims)
...
class biLstmRL(MultiHumanRL):
    ...
    def configure(self, config):
        ...
        self.with_om = config.bilstm_rl.with_om
        self.multiagent_training = config.bilstm_rl.multiagent_training

        mlp_dims = config.bilstm_rl.mlp2_dims
        global_state_dim = config.bilstm_rl.global_state_dim
        with_interaction_module = config.bilstm_rl.with_interaction_module
        if with_interaction_module:
            mlp1_dims = config.bilstm_rl.mlp1_dims
        ...
        logging.info('Policy: {}BILSTM-RL {} pairwise interaction module'.format(
