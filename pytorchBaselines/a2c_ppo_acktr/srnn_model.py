...
class RNNBase(nn.Module):
    ...
    def __init__(self, config, edge):
        ...
        if edge:
            self.gru = nn.GRU(config.PolicyConfig.SRNN.human_human_edge_embedding_size, config.PolicyConfig.SRNN.human_human_edge_rnn_size)
        # if this is a node RNN
        else:
            self.gru = nn.GRU(config.PolicyConfig.SRNN.human_node_embedding_size*2, config.PolicyConfig.SRNN.human_node_rnn_size)
...
class HumanNodeRNN(RNNBase):
    ...
    def __init__(self, config):
        ...
        self.rnn_size = config.PolicyConfig.SRNN.human_node_rnn_size
        self.output_size = config.PolicyConfig.SRNN.human_node_output_size
        self.embedding_size = config.PolicyConfig.SRNN.human_node_embedding_size
        self.input_size = config.PolicyConfig.SRNN.human_node_input_size
        self.edge_rnn_size = config.PolicyConfig.SRNN.human_human_edge_rnn_size
...
class HumanHumanEdgeRNN(RNNBase):
    ...
    def __init__(self, config):
        ...
        # Store required sizes
        self.rnn_size = config.PolicyConfig.SRNN.human_human_edge_rnn_size
        self.embedding_size = config.PolicyConfig.SRNN.human_human_edge_embedding_size
        self.input_size = config.PolicyConfig.SRNN.human_human_edge_input_size
...
class EdgeAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        # Store required sizes
        self.human_human_edge_rnn_size = config.PolicyConfig.SRNN.human_human_edge_rnn_size
        self.human_node_rnn_size = config.PolicyConfig.SRNN.human_node_rnn_size
        self.attention_size = config.PolicyConfig.SRNN.attention_size
...
class SRNN(nn.Module):
    ...
    def __init__(self, obs_space_dict, config, infer=False):
        ...
        self.human_num = config.EnvConfig.sim.human_num

        self.seq_length = config.PolicyConfig.ppo.num_steps
        self.nenv = config.TrainConfig.train.num_processes
        self.nminibatch = config.PolicyConfig.ppo.num_mini_batch
    ...
    def forward(self, inputs, rnn_hxs, masks, infer=False):
        ...
        if not self.config.TrainConfig.train.cuda:
